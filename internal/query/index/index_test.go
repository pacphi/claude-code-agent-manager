...
// BenchmarkAddAgent benchmarks agent addition performance
...
func BenchmarkSearch(b *testing.B) {
	...
	for i := 0; i < 1000; i++ {
		agent := createTestAgent(
			...
			[]string{"Read", "Write", "Edit"}[i%3:i%3+1],
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := im.Search("benchmark", QueryOptions{})
		if err != nil {
...
func BenchmarkConcurrentReads(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			_ = im.GetAll()
			_, _ = im.Search("agent", QueryOptions{})
			_ = im.GetByFilename("agent0.md")
		}
	})
}

// TestSaveAndLoad tests save/load functionality
func TestSaveAndLoad(t *testing.T) {
	tmpDir := t.TempDir()
	indexPath := filepath.Join(tmpDir, "test-index.json")

	// Create first index manager and add agents
	im1, err := NewIndexManager(indexPath)
	if err != nil {
		t.Fatalf("NewIndexManager failed: %v", err)
	}
	...
	// Create second index manager that should load from disk
	im2, err := NewIndexManager(indexPath)
	if err != nil {
		t.Fatalf("NewIndexManager failed on load: %v", err)
	}

	// Verify agents were loaded
	loadedAgents := im2.GetAll()
	if len(loadedAgents) != 2 {
		t.Errorf("Expected 2 loaded agents, got %d", len(loadedAgents))
	}

	// Verify specific agents exist
	agent1 := im2.GetByFilename("agent1.md")
	if agent1 == nil {
		t.Error("agent1 not loaded correctly - not found")
	} else if agent1.Name != "agent1" {
		t.Errorf("agent1 not loaded correctly - expected name 'agent1', got '%s'", agent1.Name)
	}

	agent2 := im2.byName["agent2"]
	if agent2 == nil || agent2.Description != "Second agent" {
		t.Error("agent2 not loaded correctly")
	}
}

// TestLoadNonexistentFile tests loading when index file doesn't exist
func TestLoadNonexistentFile(t *testing.T) {
	tmpDir := t.TempDir()
	indexPath := filepath.Join(tmpDir, "nonexistent-index.json")

	// Should create successfully even with nonexistent file
	im, err := NewIndexManager(indexPath)
	if err != nil {
		t.Fatalf("NewIndexManager should succeed with nonexistent file: %v", err)
	}

	// Should start empty
	agents := im.GetAll()
	if len(agents) != 0 {
		t.Errorf("Expected empty index for nonexistent file, got %d agents", len(agents))
	}
}

// TestSearchByDescription tests description-specific search
func TestSearchByDescription(t *testing.T) {
	...
	agents := []*parser.AgentSpec{
		createTestAgent("agent1", "Python programming expert", []string{"Read"}, "Python help"),
		createTestAgent("agent2", "Go programming specialist", []string{"Write"}, "Go help"),
		createTestAgent("agent3", "Web development with Python", []string{"Edit"}, "Web help"),
	}
	...
	results, err := im.SearchByDescription("programming")
	if err != nil {
		t.Errorf("SearchByDescription failed: %v", err)
	}

	if len(results) != 2 {
		t.Errorf("Expected 2 results for description search 'programming', got %d", len(results))
	}
}

// TestSearchByContent tests content-specific search
func TestSearchByContent(t *testing.T) {
	...
	agents := []*parser.AgentSpec{
		createTestAgent("agent1", "First agent", []string{"Read"}, "Help with Python development"),
		createTestAgent("agent2", "Second agent", []string{"Write"}, "Assistance with Go programming"),
		createTestAgent("agent3", "Third agent", []string{"Edit"}, "Support for web development"),
	}
	...
	results, err := im.SearchByContent("development")
	if err != nil {
		t.Errorf("SearchByContent failed: %v", err)
	}

	if len(results) != 2 {
		t.Errorf("Expected 2 results for content search 'development', got %d", len(results))
	}
}

// TestSearchBySource tests source-specific search
func TestSearchBySource(t *testing.T) {
	...
	agents := []*parser.AgentSpec{
		{Name: "agent1", Source: "source-a", Description: "First", Prompt: "First prompt"},
		{Name: "agent2", Source: "source-b", Description: "Second", Prompt: "Second prompt"},
		{Name: "agent3", Source: "source-a", Description: "Third", Prompt: "Third prompt"},
	}
	...
	results, err := im.SearchBySource("source-a")
	if err != nil {
		t.Errorf("SearchBySource failed: %v", err)
	}

	if len(results) != 2 {
		t.Errorf("Expected 2 results for source search 'source-a', got %d", len(results))
	}
}

// TestNewIndexManagerEmptyPath tests creating index manager with empty path
func TestNewIndexManagerEmptyPath(t *testing.T) {
	im, err := NewIndexManager("")
	if err != nil {
		t.Fatalf("NewIndexManager with empty path should succeed: %v", err)
	}
	...
	// Save should be no-op with empty path
	err = im.Save()
	if err != nil {
